...
Fingerprint::Fingerprint() {
    ...
    std::thread([this]() {
        ...
        int fodStatus = open(FOD_STATUS_PATH, O_RDWR);
        if (fodStatus < 0) {
            LOG(ERROR) << "failed to open fodStatus, err: " << fodStatus;
        }
        ...
        while (true) {
            ...
            if (fodStatus >= 0) {
                write(fodStatus, readBool(fd) ? "1" : "0", 1);
            }
        }
    }).detach();
}
...
fingerprint_device_t* Fingerprint::getSensorHal() {
    if (mDevice) {
        ...
        for (const std::string& class_name : cla) {
            ...
            SetProperty("persist.vendor.sys.fp.vendor", class_name.c_str());
